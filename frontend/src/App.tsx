...
import { CONTRACT_ADDRESS, RPC_URL, ABI, FILE_NAME, EXPLORER_URL } from './constants';
...
export default function App() {
  ...
  const [mode] = useState<Mode>((import.meta.env.VITE_LOCAL_HASH_MODE as Mode) || 'manual');
  ...
  const statusColor = isVerified === null ? 'text-gray-500' : isVerified ? 'text-ui-success' : 'text-ui-error';
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 max-w-7xl mx-auto flex flex-col gap-8">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-gray-500 font-medium tracking-wider uppercase">Security Infrastructure</p>
          </div>
        </div>
        <div className="flex flex-wrap gap-3">
          ...
          >
            ...
            <svg className="w-3 h-3 ml-2 opacity-30 group-hover:opacity-100 transition-opacity" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <main className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* Status Card */}
        <section className={`lg:col-span-7 bg-ui-card border border-ui-border rounded-2xl p-12 flex flex-col items-center justify-center relative overflow-hidden shadow-xl shadow-black/20 transition-all duration-500 ${isVerified === false ? 'ring-1 ring-ui-error/20' : ''}`}>
          <div className="absolute top-6 left-8 text-[10px] font-bold text-gray-600 tracking-[0.2em] uppercase">Integrity Status</div>

          {isLoading ? (
            <div className="flex flex-col items-center gap-4">
              <div className="w-12 h-12 border-2 border-brand-500/20 border-t-brand-500 rounded-full animate-spin"></div>
              <div className="text-sm font-medium text-gray-500 uppercase tracking-widest">Analyzing...</div>
            </div>
          ) : (
            <div className="text-center">
              <div className={`text-6xl md:text-8xl font-bold mb-6 tracking-tight ${statusColor} ${statusAnimation}`}>
                {isVerified === null ? 'IDLE' : isVerified ? 'VERIFIED' : 'TAMPERED'}
              </div>
              <div className="flex items-center justify-center gap-2">
                 {isVerified === true && <svg className="w-5 h-5 text-ui-success" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" /></svg>}
                 {isVerified === false && <svg className="w-5 h-5 text-ui-error" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" /></svg>}
                 <div className="text-sm font-medium text-gray-400">
                  {isVerified === null ? 'Awaiting comparison data' : isVerified ? 'All security checks passed' : 'Hash mismatch detected'}
                </div>
              </div>
            </div>
          )}
        ...
        <div className="lg:col-span-5 flex flex-col gap-6">
          ...
          <div className="bg-ui-card border border-ui-border rounded-xl p-6 shadow-lg">
            <div className="flex justify-between items-center mb-6">
              <span className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">On-Chain Record</span>
              <span className="px-2 py-0.5 rounded bg-brand-500/10 text-brand-500 text-[10px] font-bold uppercase">Live State</span>
            </div>
            <div className="space-y-6">
              <div>
                <label className="block text-[10px] font-semibold text-gray-600 uppercase mb-2">Latest Verified Hash</label>
              ...
              <div className="grid grid-cols-2 gap-4 pt-2">
                <div>
                  <label className="block text-[10px] font-semibold text-gray-600 uppercase mb-1">Signer Role</label>
                  <div className="text-[11px] font-medium text-white">System Administrator</div>
                </div>
                <div>
                  <label className="block text-[10px] font-semibold text-gray-600 uppercase mb-1">Resource ID</label>
                  <div className="text-[11px] font-mono text-gray-400">{fileId.slice(0, 12)}...</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-ui-card border border-ui-border rounded-xl p-6 shadow-lg">
            <div className="flex justify-between items-center mb-6">
              <span className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">Local Environment</span>
              <span className="px-2 py-0.5 rounded bg-gray-800 text-gray-400 text-[10px] font-bold uppercase">{mode} Mode</span>
            </div>

            {mode === 'manual' ? (
              <div className="space-y-6">
                <div>
                  <label className="block text-[10px] font-semibold text-gray-600 uppercase mb-2 text-center">Validate Resource: {FILE_NAME}</label>
                  <div className="relative group">
                    ...
                    <div className="border-2 border-dashed border-ui-border rounded-lg p-4 text-center group-hover:border-brand-500/50 transition-colors bg-white/[0.02]">
                      <span className="text-xs text-gray-400">Drop file or <span className="text-brand-500">browse</span></span>
                    </div>
                  </div>
                </div>
                <div className="relative">
                  <div className="absolute inset-y-0 left-3 flex items-center pointer-events-none">
                    <span className="text-[10px] text-gray-600 font-mono">0x</span>
                  </div>
                  <input
                    type="text"
                    value={localHash}
              ...
              <div className="flex flex-col items-center justify-center py-8 bg-white/[0.02] rounded-lg border border-dashed border-ui-border">
                ...
                <div className="text-[11px] font-medium text-gray-400 mb-3 tracking-wide uppercase">Monitoring Filesystem</div>
      ...
      <footer className="flex flex-col md:flex-row justify-between items-center gap-4 py-8 border-t border-ui-border">
        ...
        <p className="text-[10px] text-gray-600 max-w-sm text-center md:text-right">
          Hashes are immutable once committed to the Monad network. Discrepancies indicate unauthorized local modifications.
        </p>
      </footer>
    </div>
  );
